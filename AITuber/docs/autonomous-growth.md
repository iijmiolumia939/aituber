...
#### 問題3: Phase 2のLLM出力を検証するバリデーターが未定義 ⚠️
...
| コンポーネント | 技術 | 配置 | フェーズ |
|---|---|---|---|
| ActionDispatcher | C# (Unity) | `Assets/Scripts/Growth/ActionDispatcher.cs` | Phase 1 |
| GapLogger | C# (Unity) | `Assets/Scripts/Growth/GapLogger.cs` | Phase 1 |
| GapDashboard | Python (click + rich) | `tools/gap_dashboard.py` | Phase 1 |
| ReflectionRunner | Python (LLM client) | `tools/growth/reflection_runner.py` | Phase 1 |
| BehaviorPolicy | YAML + C# loader | `Assets/StreamingAssets/behavior_policy.yml` | Phase 1 |
...
| Milestone | 内容 | 依存 | 目安 |
|---|---|---|---|
| M1 | `ActionDispatcher` + `GapLogger` 実装・ログ収集開始（[設計書](m1-design.md)） | — | 近日 |
| M2 | `ReflectionRunner` 実装・初回Reflection実行 | M1 | M1+1週 |
| M3 | `GapDashboard` で初回集計・上位5 Gap特定・Issue作成 | M2 | M2+1週 |
