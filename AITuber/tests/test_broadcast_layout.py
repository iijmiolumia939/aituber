...
def _make_manager(
    *,
    ...
    obs_ok: bool = True,
) -> tuple[BroadcastLayoutManager, MagicMock, MagicMock, MagicMock, MagicMock]:
    """Return (manager, send_scene, send_layout, send_transition, obs_switch)."""
    ...
    return mgr, send_scene, send_layout, send_transition, obs_switch
...
def test_layout_04_transition_in_called_first():
    ...
    mgr, _, _, send_trans, obs_switch = _make_manager()
...
def test_layout_05_obs_switch_called_with_obs_scene_name():
    ...
    mgr, _, _, _, obs_switch = _make_manager()
    _run(mgr.switch_to(BroadcastScene.OPENING))
...
def test_layout_06_send_scene_called_with_slug():
    ...
    mgr, send_scene, _, _, _ = _make_manager()
...
def test_layout_07_send_layout_called():
    ...
    mgr, _, send_layout, _, _ = _make_manager()
...
def test_layout_08_transition_out_called_last():
    ...
    mgr, send_scene, _, send_trans, _ = _make_manager()
...
def test_layout_12_update_game_frame_sends_layout_only():
    ...
    mgr, send_scene, send_layout, _, obs_switch = _make_manager()
